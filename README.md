...
│   │   ├── 00_import_sc_rna_seq_data.R
│   │   ├── 01_heatmap_single_cell.R
│   │   ├── 02_genes_violin.R
│   │   └── 03_UMAP.R
...
├── .gitignore
├── sars-cov2-upper-airway-age-signature.Rproj
└── README.md
```

This repository intentionally contains **code only** — no `results/` folder
is committed. `.gitignore` excludes generated tables, figures, and any raw
or intermediate data (CSV/TSV, images, PDFs, RDS/RData, sequencing files,
etc.); running the scripts below will recreate a local `results/` tree with
the same layout used during the round-3 manuscript resubmission
(`results/deseq2/<GSE...>/{primary,sensitivity,diagnostics,metadata}/`,
`results/meta_analysis/...`, `results/gsea/...`,
`results/scRNAseq_COVID_nasal_age/...`), but those outputs are not tracked
by git and must be regenerated by running the pipeline.

Note: the script that generated the PCA/forest-plot panels used in main
Figure 1 is not present under `scripts/05_figures/`; that figure was
assembled from `*_PCA_scores.csv` diagnostics generated by the
`01_deseq2/` scripts and the pooled forest-plot panel. Add the
corresponding script here when it is finalized, so that all four main
figures are fully reproducible from this repository.
...
scripts/04_single_cell_rna_seq/00_import_sc_rna_seq_data.R
scripts/04_single_cell_rna_seq/01_heatmap_single_cell.R
scripts/04_single_cell_rna_seq/02_genes_violin.R
scripts/04_single_cell_rna_seq/03_UMAP.R
...
writes its outputs to a local `results/` subfolder (git-ignored, see above).
...
Complete `sessionInfo()` output for every analysis stage is not committed to
this repository (git-ignored, like all other generated output) but is
...
interpreting any output the pipeline generates locally under `results/`:
